...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'role', 'school', 'school_name', 'profile_photo', 'is_active', 'date_joined'
        ]
    ...
    def validate(self, attrs):
        ...
        if request and request.user.is_authenticated:
            if request.user.is_superuser and not attrs.get('school'):
                raise serializers.ValidationError({'school': 'School is required when creating users as a superuser.'})
            if not request.user.is_superuser and not request.user.school:
                raise serializers.ValidationError({'school': 'Your account is not linked to a school. Assign a school to this admin account first.'})
        return attrs
...
class UserCreatePasswordRetypeSerializer(DjoserUserCreatePasswordRetypeSerializer):
    ...
    class Meta(DjoserUserCreatePasswordRetypeSerializer.Meta):
        fields = DjoserUserCreatePasswordRetypeSerializer.Meta.fields + ('role', 'school')
