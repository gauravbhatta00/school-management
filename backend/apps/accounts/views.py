...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return (
            User.objects.filter(school=self.request.user.school)
        ...
        recent_teachers = [
            {
                ...
                "qualification": getattr(
                    getattr(teacher, "teacher_profile", None), "qualification", ""
                ),
                "experience_years": getattr(
                    getattr(teacher, "teacher_profile", None), "experience_years", None
                ),
            }
    ...
    def student_portal(self, request):
        if request.user.role != User.Role.STUDENT:
            return Response(
                {"error": "Only students can access this endpoint."}, status=403
            )
        ...
        results_qs = (
            ...
            .order_by("-exam__start_date", "-created_at")
        )
        ...
        recent_payments = list(
            payments_qs.values(
                "id", "amount", "status", "payment_date", "payment_method"
            )[:10]
        )
        ...
        return Response(
            {
                ...
                "attendance": {
                    ...
                    "attendance_rate": round(
                        attendance_present / attendance_total * 100, 2
                    )
                    if attendance_total
                    else 0,
