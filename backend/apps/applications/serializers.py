...
class ApplicationListSerializer(serializers.ModelSerializer):
    ...
    teacher_name = serializers.CharField(source='teacher_reviewed_by.get_full_name', read_only=True, allow_null=True)
    admin_name = serializers.CharField(source='admin_reviewed_by.get_full_name', read_only=True, allow_null=True)

    class Meta:
        model = Application
        fields = [
            'id', 'student_id', 'student_name', 'application_type', 'application_type_display',
            'status', 'status_display', 'title', 'description', 'start_date', 'end_date',
            'teacher_decision', 'teacher_reviewed_at', 'teacher_name',
            'admin_decision', 'admin_reviewed_at', 'admin_name',
            'created_at'
        ]
...
class ApplicationDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Application
        fields = [
            'id', 'student_id', 'student_name', 'student_email', 'application_type', 'application_type_display',
            'status', 'status_display', 'title', 'description', 'start_date', 'end_date',
            'teacher_review', 'teacher_decision', 'teacher_reviewed_at', 'teacher_reviewer',
            'admin_review', 'admin_decision', 'admin_reviewed_at', 'admin_reviewer',
    ...
    def validate(self, attrs):
        ...
        if app_type == Application.Type.LEAVE_REQUEST:
            if not attrs.get('start_date') or not attrs.get('end_date'):
                raise serializers.ValidationError("Start and end dates are required for leave requests.")
...
class TeacherApplicationReviewSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        with transaction.atomic():
            instance.teacher_review = validated_data.get('teacher_review', instance.teacher_review)
            instance.teacher_decision = validated_data.get('teacher_decision', instance.teacher_decision)
...
class AdminApplicationReviewSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        request = self.context.get('request')
        instance.admin_review = validated_data.get('admin_review', instance.admin_review)
        instance.admin_decision = validated_data.get('admin_decision', instance.admin_decision)
