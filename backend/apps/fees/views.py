...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in ['destroy']:
            return [IsAuthenticated(), IsSchoolAdmin()]
        if self.action in ['create', 'update', 'partial_update', 'pay', 'collection_summary']:
            return [IsAuthenticated(), IsAdminOrTeacher()]
    ...
    def _build_fee_report_rows(self, school, academic_year, class_name='', search=''):
        students_qs = Student.objects.filter(school=school).select_related('user').order_by(
            'class_name', 'section', 'roll_number', 'user__first_name', 'user__last_name'
        )
    ...
    def pay(self, request):
        ...
        try:
            fee_structure = FeeStructure.objects.get(id=d['fee_structure_id'], school=school)
        ...
        paid_so_far = Payment.objects.filter(
            ...
            fee_structure=fee_structure, status__in=['paid', 'partial']
    ...
    def student_status(self, request):
        ...
        if request.user.role == 'student' and student.user_id != request.user.id:
            return Response({'error': 'You can only access your own fee status.'}, status=403)
    ...
    def collection_summary(self, request):
        ...
        payments = Payment.objects.filter(school=school, fee_structure__academic_year=academic_year)
    ...
    def fee_report(self, request):
        ...
        school_summary = {
            ...
            'partial_students': sum(1 for r in school_rows if r['fee_status'] == 'partial'),
            'pending_students': sum(1 for r in school_rows if r['fee_status'] == 'pending'),
            'no_structure_students': sum(1 for r in school_rows if r['fee_status'] == 'no_structure'),
        }
        ...
        class_breakdown = sorted(class_breakdown_map.values(), key=lambda x: str(x['class_name']))

        if export == 'csv':
            response = HttpResponse(content_type='text/csv')
            response['Content-Disposition'] = f'attachment; filename="fee_report_{academic_year}.csv"'

            writer = csv.writer(response)
            writer.writerow([
                'Student Name', 'Email', 'Class', 'Section', 'Roll Number',
                'Academic Year', 'Fee Required', 'Total Paid', 'Balance', 'Status'
            ])
            for row in rows:
                writer.writerow([
                    row['student_name'],
