...
class StudentViewSet(viewsets.ModelViewSet):
    ...
    def import_csv(self, request):
        ...
        if not csv_file:
            return Response({'detail': 'CSV file is required under "file".'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({'detail': 'Unable to decode CSV file. Use UTF-8 encoding.'}, status=status.HTTP_400_BAD_REQUEST)

        reader = csv.DictReader(io.StringIO(decoded))
        required_columns = {'email', 'first_name', 'last_name', 'class_name', 'section', 'roll_number'}
        ...
        for index, row in enumerate(reader, start=2):
            ...
            if not all([email, first_name, last_name, class_name, section, roll_number]):
                failed_count += 1
            ...
            if User.objects.filter(email=email).exists():
                failed_count += 1
                errors.append({'row': index, 'email': email, 'error': 'Email already exists.'})
                continue
        ...
        return Response(
            {
                ...
                'errors': errors,
                'required_columns': sorted(required_columns),
                'optional_columns': ['parent_contact', 'address', 'date_of_birth', 'password'],
            },
